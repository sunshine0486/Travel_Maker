...
import { getBoardDtl } from "../boardApi";
      ...
      try {
        ...
        const files: FileItem[] =
          data.boardFileDtoList?.map((f) => ({
            name: f.oriFileName,
            size: f.fileSize,
            type: "", // 실제 File 객체가 없으면 빈 문자열
            file: new File([], f.oriFileName), // UI용
          })) || [];
...
//   const [category, setCategory] = useState("");
//   const [title, setTitle] = useState("");
//   const [content, setContent] = useState(""); // QuillEditor value
//   const [hashtags, setHashtags] = useState<string[]>([]);
//   const [inputValue, setInputValue] = useState("");
...
//     formData.append("hashTag", hashtags.join(""));
//     files.forEach((f) => formData.append("boardFile", f.file));
